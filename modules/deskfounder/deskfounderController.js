...
exports.addDeskfounder = async (req, res) => {
  try {
    ...
    res.status(200).json({ msg: "add successfuly", data:Deskfounder });
  } catch (error) {
    console.error(error);
...
//     res
//       .status(400)
//       .send({ msg: "Couldn't Retrieve deskfounder", details: error });
//   }
// };
exports.getDeskfounder = async (req, res) => {
  try {
...
exports.updateDeskfounder = async (req, res) => {
  ...
  try {
    ...
    const updateDeskfounder = await deskfounderModel.findByIdAndUpdate(
      id,
      updateData,
      { new: true }
    );
    res.status(200).send({msg:"Update Successfuly",data:updateDeskfounder});
  } catch (error) {
    console.error(error);
